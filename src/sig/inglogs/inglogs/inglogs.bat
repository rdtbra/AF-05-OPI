...
REM
REM 20-Jun-2011 Bilgihan Bircan
REM   v1.04
REM   added infodb outputs
REM   added echo STACKS |iimonitor %1 > stacks.%EXT%.txt
REM         Depending of the language and region setting inglogs is not able 
REM            creating unique directory names and not always in format 
REM            yyyymmdd_hhmmss
REM 01-Jul-2011 Paul Mason
REM   Make the name of the inglogs directory consistent with unix and VMS 
REM   versions


set INGLOGS_VERSION=1.04_WIN
...
REM Create directory name (DNAME) of INGLOGS_YYYYMMDD_HHMMSS
REM Regardless of which language and region have been configured for Windows
...
REM DOS does not add leading zero.
REM Add '0' if the time before 10 o'clock in the morning.
...
set DNAME=INGLOGS_%V_Year%%V_Month%%V_Day%_%hh%%mn%%ss%%ms%
...
if "%SVRDONE%"=="0" (
  ...
  echo STACKS                        |iimonitor %1 > stacks.%EXT%.txt
