...
CS_setup(void)
{
  ...
  if (EXdeclare(cs_handler, &excontext))
  {
	  ...
	  {
	    (*Cs_srv_block.cs_elog)(E_CS00FF_FATAL_ERROR, (DB_ERROR *)NULL,
        NULL, 0);
	    EXdelete();
	  }
	  else
  ...
  */
  ...
  if ((scb != (CS_SCB *)&Cs_admin_scb) && (scb != (CS_SCB *)&Cs_idle_scb))
  {
	  ...
	  for (;;)
	  {
	    switch (scb->cs_mode)
	    {
		    ...
		    case CS_READ:
		      break;
        default:
		      (*Cs_srv_block.cs_elog)(E_CS000C_INVALID_MODE, 
	    ...
	    if ((scb->cs_mask & CS_DEAD_MASK) == 0)
	    {
		    switch (scb->cs_nmode)
		    {
		      ...
		      case CS_EXCHANGE:
			      ...
			      break;
		      default:
			      (*Cs_srv_block.cs_elog)(E_CS000D_INVALID_RTN_MODE, 
              (DB_ERROR *)NULL, NULL, 0);
			      scb->cs_mode = CS_TERMINATE;
			      break;
		    }
	  ...
	  if (scb->cs_sem_count)
	  {
	    (*Cs_srv_block.cs_elog)(E_CS001C_TERM_W_SEM, (DB_ERROR *)NULL,
         NULL, 0);
	  }
	  EXdelete();
  }
...
CS_rcv_request(void)
{
	...
	(*Cs_srv_block.cs_elog)(E_CS0011_BAD_CNX_STATUS, (DB_ERROR *)NULL, 
                                &err, 0);
	return(FAIL);
    }
    return(OK);
...
CS_del_thread(CS_SCB *scb)
{
    ...
    */
    if (acctstat != OK)
        (*Cs_srv_block.cs_elog)(acctstat, (DB_ERROR *)NULL, NULL, 0);
#ifdef MCT
...
CS_admin_task(i4 mode, CS_SCB *ascb, i4 *next_mode, PTR io_area)
{
    ...
    {
	(*Cs_srv_block.cs_elog)(E_CS0012_BAD_ADMIN_SCB, (DB_ERROR *)NULL, 
                                NULL, 0);
	return(FAIL);
    }
