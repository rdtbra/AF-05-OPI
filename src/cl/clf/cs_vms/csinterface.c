...
**          Modified Ex_diag_link to return an integer. A return of 0
**          indicates the server does not have sessions. 1 implies
**          a CS_IDLE server (like a DBMS).
**      20-Jan-2011 (horda03)
**          Inside a threaded server reduce the frequeny of SEM WAIT messages.
**          Now that processes are getting faster using loop counters
**          isn't enough to delay the message until the session has waited a
**          second or two. Inside a threaded server use the CS_QUANTUM.
**     13-Jul-2011 (hanal04) SIR 125458
**         Update cs_elog calls inline with sc0e_putAsFcn changes.
**      09-Aug-2011 (horda03) b125459
**          Use CS_Stall defined in cslocal.h
**/

/*
...
CSinitiate(i4 *argc, char ***argv, CS_CB *ccb)
{
    ...
    */
    ...
    {
	(*Cs_srv_block.cs_elog)(E_CS0016_SYSTEM_ERROR, (DB_ERROR *)NULL, 
                                &status, 0);
	sys$exit(status);
    }
}
...
i4      *active_count)
{
    ...
    */
	...
	if (status)
	{
	    (*Cs_srv_block.cs_elog)(status, (DB_ERROR *)NULL, NULL, 0);
	}
    }
    if (mode != CS_CRASH)
    {
	...
	if (status)
	    ...
	    (*Cs_srv_block.cs_elog)(E_CS0018_NORMAL_SHUTDOWN, (DB_ERROR *)NULL,
                                    NULL, 0);
    }
    PCexit(OK);
}
...
IICSdispatch()
{
	...
	if (!got_ex++)
	{
	    (*Cs_srv_block.cs_elog)(E_CS0014_ESCAPED_EXCPTN, (DB_ERROR *)NULL,
                                    NULL, 0);
	}
	EXdelete();
		...
		*/
		Cs_srv_block.cs_toq_cnt = 
			...
			- (Cs_srv_block.cs_bquantum[0] / CS_ONE_SECOND);
                ...
                {
                    (*Cs_srv_block.cs_elog)(E_CS0010_QUANTUM_INVALID, 
                                            (DB_ERROR *)NULL, NULL, 0);
                    EXdelete();
...
TRdisplay("->->->->\n");
	...
	*/
	...
	(*Cs_srv_block.cs_elog)(E_CS00FE_RET_FROM_IDLE, (DB_ERROR *)NULL,
                                NULL, 0);
	...
	(*Cs_srv_block.cs_elog)(ret_val, (DB_ERROR *)NULL, &status, 0);
    }
...
**             have timedout.
**      20-Jan-2011 (horda03)
**         Enable ASTs here, then if some QIO has already completed, there
**         won't be a need to reschedule.
*/
STATUS
IICSsuspend(
i4      mask,
...
PTR     ecb)
{
    ...
    {
        ast_enabled = (sys$setast(1) == SS$_WASSET);
	...
	*/
	...
	else
	{
	    ...
	    (*Cs_srv_block.cs_elog)(E_CS0019_INVALID_READY, (DB_ERROR *)NULL,
                                    NULL, 0);
        ...
        int     fac_id)
{
    ...
    else /* end counting */
    {
        if (Cs_srv_block.cs_profile[fac_id].cs_active == FALSE)
        {
            ...
            (*Cs_srv_block.cs_elog)(E_CS0046_CS_NO_START_PROF, (DB_ERROR *)NULL,
                                    NULL, 1,
...
CS_SEMAPHORE       *sem)
{
        ...
        */
        ...
        if (scb)
        {
	      ...
	      retune_priority = FALSE;
              warn_quantum = Cs_srv_block.cs_quantums + 5; /* Wait 5 seconds */
        }
        else
        {
              warn_quantum = 0;
	...
	for (real_loops = 0; ; real_loops++)
		    ...
		    */
                 ...
                 if (warn_msg)
                 {
                    ...
                    STprintf(err_buffer+STlength(err_buffer),
                             "Sem Granted: pid %x ses %x, spid %x sses %x adr %x t %d s %d c %d %0.30s ",
		 ...
		 cpu_yields = 0;
           ...
           */
                  ...
                  (trace_count++ > Cs_max_sem_loops) ))
           {
               ...
               if (warn_quantum)
               {
                    if (warn_quantum < Cs_srv_block.cs_quantums) disp_msg = 1;
               }
               else if (inner_trace_count++ > Cs_max_sem_loops)
               {
                    disp_msg = 1;
        ...
        sem->cs_test_set, sem->cs_count, sem->cs_sem_name);
                    ...
                    owner_pid = sem->cs_pid;
                    owner_scb = sem->cs_owner;
                }
           }
           else if (warn_msg && ( (owner_pid != sem->cs_pid) || (owner_scb != sem->cs_owner)) )
           {
              warn_msg = 0;
              if (warn_quantum) warn_quantum = Cs_srv_block.cs_quantums + 5;
           }
...
#endif
	{
		...
		sys$setast(1);
            ...
            TRdisplay("failed to allocate session stack, status = %x\n",
                      status);
            (*Cs_srv_block.cs_elog)(status, (DB_ERROR *)NULL, error, 0);
...
CS_SEMAPHORE	*mtx)
{
    ...
    {
	CS_stall(FALSE, scb);	/* wait we are restarted and are runable */
...
i4		server_stats)
{
    ...
    */
    ...
    if (( i & 1) == 0)
    {
	(*Cs_srv_block.cs_elog)(E_CS0016_SYSTEM_ERROR, (DB_ERROR *)NULL, &i, 0);
...
PTR		item)
{
	...
	if (item && (*(i4 *)item))
	{
		...
		*/
		...
		if (( i & 1) == 0)
		    (*Cs_srv_block.cs_elog)(E_CS0016_SYSTEM_ERROR, 
                                            (DB_ERROR *)NULL, &i, 0);
		else
...
IICSswitch()
{
	/* RDT: 2021-05-12 - Não sei se a função chamada pela CS_swuser, no ponto onde estive avaliando, chama esta função IICSswitch(). Em algum ponto encontrei uma atribuição de
	   ponteiro de função, mas na análise inicial que faço deste código, suponho que seja este código executado. */
  if (Cs_srv_block.cs_state != CS_INITIALIZING)
  {
    if ((Cs_srv_block.cs_current->cs_thread_type != CS_INTRNL_THREAD) ||
       (CS_lastquant > ALLOWED_QUANTUM_COUNT) )
    {
    	/* RDT: 2021-05-12 - Vencidos os limites de contagem aqui do if acima, chamaremos esta função CS_stall(). Em minhas suposições iniciais, entendo que estamos indo a 
    	   um nível mais baixo deste código CS - que também suponho seja a abreviação de Control Server!? */
      CS_stall( FALSE, Cs_srv_block.cs_current );
    }
...
CS_swuser()
{
	/* RDT: 2021-05-12 - CS_swuser executa nada mais do que uma troca de contexto voluntária. Provavelmente este é o ponto do servidor Ingres, algo parecido com o que 
	   vemos no Informix até hoje. */
	CSswitch();
	return;
}

