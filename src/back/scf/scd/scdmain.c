...
**	4-Jun-2011 (chris.dawe@iua.org.uk) SIR 125407
**	    Allow '%d' (date YYYYMMDD) for II_DBMS_LOG
**      14-Jun-2011 (hanal04) SIR 125429
**          Move %p and %d log file name substitutions down into TRset_file().
...
char	**argv;
{
  ...
  */
  ...
  /*
  ** TRset_file() will change file-name if $II_DBMS_LOG contains %p 
  ** (process-id) and/or %d (date 'YYYYMMdd')
  */
  if ((tran_place) && (*tran_place))
  {
    TRset_file(TR_F_OPEN, tran_place, STlength(tran_place), &cl_err);
  }
...
**	23-Mar-2011 (kschendel) b125204 m1782
**	    sscb_cpy_qeccb deleted, fix here.
**	24-May-2011 (kschendel) m1260
**	    Allocate x100cb (aka X100SessionState) in the SCB.
*/
STATUS
scd_alloc_scb( CS_SCB  **csscbp, void  *input_crb, i4  thread_type )
{
	...
	*/
	...
	if ( crb->gca_status != E_GCFFFE_INCOMPLETE )
	{
		...
		sc0ePut(crb->gca_status, (DB_ERROR *)NULL, 
                        &crb->gca_os_status, 0);
    ...
    */
	...
	mem_needed = DB_ALIGN_MACRO(mem_needed);
#if defined(conf_IVW)
	offset_x100 = mem_needed;
	mem_needed += sizeof(struct X100SessionState);
	mem_needed = DB_ALIGN_MACRO(mem_needed);
#endif
    }
...
#if defined(conf_IVW)
	scb->scb_sscb.sscb_x100cb = (struct X100SessionState *) (block + offset_x100);
	MEfill(sizeof(struct X100SessionState), 0, scb->scb_sscb.sscb_x100cb);
#else
	scb->scb_sscb.sscb_x100cb = NULL;
#endif
    }
...
scd_reject_assoc( void *parm, STATUS error )
{
    ...
    if ( error )
	sc0ePut( error, (DB_ERROR *)NULL, NULL, 0 );

    if (crb->gca_status && crb->gca_status != E_GC0000_OK)
    {
	if (crb->gca_status != E_GC0032_NO_PEER)
	    sc0ePut(crb->gca_status, (DB_ERROR *)NULL, &crb->gca_os_status, 0);
	...
	sc0ePut(gca_status, (DB_ERROR *)NULL, &rrparms.gca_os_status, 0);
...
# endif
	    )
    {
        ...
        */
        ...
        {
	    sc0ePut(E_SC023A_GCA_LISTEN_FAIL, (DB_ERROR *)NULL, NULL, 0);
...
scd_disconnect( CS_SCB  *csscb )
{
    ...
    if (status)
    {
	sc0ePut(gca_status, (DB_ERROR *)NULL, NULL, 0);
    }
}
...
{
    sc0ePut(err_number, (DB_ERROR *)NULL, NULL, num_arguments, args);
}
